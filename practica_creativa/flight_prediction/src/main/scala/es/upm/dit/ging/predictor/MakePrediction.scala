...
object MakePrediction {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // define a streaming query (mongo)
    ...
    // define a streaming query (kafka)
    val kafkaOutput = finalPredictions // cambiamos el formato de las predicciones para guardarlas en kafka
      .withColumn("key", col("UUID").cast("string"))
      .withColumn("value", to_json(struct("*")))

    val dataStreamWriterKafka = kafkaOutput
      .writeStream
      .outputMode("append")
      .option("checkpointLocation", "/tmp/checkpoint-kafka-result")
      .foreachBatch { (batchDF: Dataset[Row], batchId: Long) =>
        ...
        batchDF
          ...
          .format("kafka")
          .option("kafka.bootstrap.servers", "kafka:9092")
          .option("topic", "flight-delay-ml-result")
          .save()
      }
    ...
    val hdfsOutput = finalPredictions
      ...
      .format("csv") 
      .outputMode("append")
      .option("path", "hdfs://hadoop-namenode:8020/user/spark/predictions") 
    ...
    // run the query (kafka)
    val queryKafka = dataStreamWriterKafka.start()

    // run the query (HDFS)
    val queryHDFS = hdfsOutput.start()
